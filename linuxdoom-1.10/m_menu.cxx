...
void M_ReadSaveStrings(void)
{
    ...
    for (i = 0; i < load_end; i++)
    {
        ...
        // JONNY TODO handle = fopen (name, O_RDONLY | 0);
        if (handle == -1)
        {
        ...
        // JONNY TODO count = read (handle, &savegamestrings[i], SAVESTRINGSIZE);
...
void M_DrawLoad(void)
{
    ...
    V_DrawPatchDirect(72, 28, 0, (patch_t *)W_CacheLumpName("M_LOADG"));
    for (i = 0; i < load_end; i++)
    {
...
void M_DrawSaveLoadBorder(int x, int y)
{
    ...
    V_DrawPatchDirect(x - 8, y + 7, 0, (patch_t *)W_CacheLumpName("M_LSLEFT"));

    for (i = 0; i < 24; i++)
    {
        V_DrawPatchDirect(x, y + 7, 0, (patch_t *)W_CacheLumpName("M_LSCNTR"));
        x += 8;
    }

    V_DrawPatchDirect(x, y + 7, 0, (patch_t *)W_CacheLumpName("M_LSRGHT"));
}
...
void M_DrawSave(void)
{
    ...
    V_DrawPatchDirect(72, 28, 0, (patch_t *)W_CacheLumpName("M_SAVEG"));
    for (i = 0; i < load_end; i++)
    {
        M_DrawSaveLoadBorder(LoadDef.x, LoadDef.y + LINEHEIGHT * i);
        M_WriteText(LoadDef.x, LoadDef.y + LINEHEIGHT * i, savegamestrings[i]);
...
void M_DrawReadThis1(void)
{
    ...
    {
    case commercial:
        V_DrawPatchDirect(0, 0, 0, (patch_t *)W_CacheLumpName("HELP"));
        break;
    case shareware:
    case registered:
    case retail:
        V_DrawPatchDirect(0, 0, 0, (patch_t *)W_CacheLumpName("HELP1"));
        break;
    default:
        break;
    }
...
void M_DrawReadThis2(void)
{
    ...
    case commercial:
        ...
        V_DrawPatchDirect(0, 0, 0, (patch_t *)W_CacheLumpName("CREDIT"));
        break;
    case shareware:
    case registered:
        V_DrawPatchDirect(0, 0, 0, (patch_t *)W_CacheLumpName("HELP2"));
        break;
    default:
        break;
    }
...
void M_DrawSound(void)
{
    V_DrawPatchDirect(60, 38, 0, (patch_t *)W_CacheLumpName("M_SVOL"));
...
// M_DrawMainMenu
//
void M_DrawMainMenu(void) { V_DrawPatchDirect(94, 2, 0, (patch_t *)W_CacheLumpName("M_DOOM")); }
...
void M_DrawNewGame(void)
{
    V_DrawPatchDirect(96, 14, 0, (patch_t *)W_CacheLumpName("M_NEWG"));
    V_DrawPatchDirect(54, 38, 0, (patch_t *)W_CacheLumpName("M_SKILL"));
}
...
void M_DrawEpisode(void) { V_DrawPatchDirect(54, 38, 0, (patch_t *)W_CacheLumpName("M_EPISOD")); }
...
void M_DrawOptions(void)
{
    V_DrawPatchDirect(108, 15, 0, (patch_t *)W_CacheLumpName("M_OPTTTL"));

    V_DrawPatchDirect(OptionsDef.x + 175, OptionsDef.y + LINEHEIGHT * detail, 0, (patch_t *)W_CacheLumpName(detailNames[detailLevel]));

    V_DrawPatchDirect(OptionsDef.x + 120, OptionsDef.y + LINEHEIGHT * messages, 0, (patch_t *)W_CacheLumpName(msgNames[showMessages]));
...
void M_DrawThermo(int x, int y, int thermWidth, int thermDot)
{
    ...
    xx = x;
    V_DrawPatchDirect(xx, y, 0, (patch_t *)W_CacheLumpName("M_THERML"));
    xx += 8;
    for (i = 0; i < thermWidth; i++)
    {
        V_DrawPatchDirect(xx, y, 0, (patch_t *)W_CacheLumpName("M_THERMM"));
        xx += 8;
    }
    V_DrawPatchDirect(xx, y, 0, (patch_t *)W_CacheLumpName("M_THERMR"));

    V_DrawPatchDirect((x + 8) + thermDot * 8, y, 0, (patch_t *)W_CacheLumpName("M_THERMO"));
}

void M_DrawEmptyCell(menu_t *menu, int item) { V_DrawPatchDirect(menu->x - 10, menu->y + item * LINEHEIGHT - 1, 0, (patch_t *)W_CacheLumpName("M_CELL1")); }

void M_DrawSelCell(menu_t *menu, int item) { V_DrawPatchDirect(menu->x - 10, menu->y + item * LINEHEIGHT - 1, 0, (patch_t *)W_CacheLumpName("M_CELL2")); }
...
void M_Drawer(void)
{
    ...
    for (i = 0; i < max; i++)
    {
        if (currentMenu->menuitems[i].name[0])
            V_DrawPatchDirect(x, y, 0, (patch_t *)W_CacheLumpName(currentMenu->menuitems[i].name));
        y += LINEHEIGHT;
    }

    // DRAW SKULL
    V_DrawPatchDirect(x + SKULLXOFF, currentMenu->y - 5 + itemOn * LINEHEIGHT, 0, (patch_t *)W_CacheLumpName(skullName[whichSkull]));
}
