...
//  the WAD file.
//
std::array<animdef_t, 22> animdefs = {
...
//
side_t *getSide(int currentSector, int line, int side) { return &sides[(sectors[currentSector].lines[line])->sidenum[side]]; }
...
//  the line number and the side (0/1) that you want.
//
sector_t *getSector(int currentSector, int line, int side) { return sides[(sectors[currentSector].lines[line])->sidenum[side]].sector; }
...
int EV_DoDonut(line_t *line)
{
    ...
    while ((secnum = P_FindSectorFromLineTag(line, secnum)) >= 0)
    {
        ...
        {
            if ((int(!s2->lines[i]->flags) & ML_TWOSIDED) || (s2->lines[i]->backsector == s1))
                continue;
